...
class Item:
    ...
    def __init__(self, next_hop, distance):
        self.next_hop = next_hop
        self.distance = distance
        self.type = Item.DYNAMIC

    def __hash__(self):
        return hash(self.next_hop) + hash(self.distance)

    def __str__(self):
        return f'{self.next_hop} - {self.distance}'


class RouteTable:
    def __init__(self, self_port, inf):
        self.inf = inf
        self.table = {self_port: Item(next_hop='self', distance=0)}

    def update_table(self, table, neighbor_port):
        updated = False
        for port, item in table.table.items():
            distance_new = min(self.inf, item.distance + 1)
            if port not in self.table:
                ...
                if self.table[port].type == Item.STATIC:
                    continue
                if self.table[port].next_hop == neighbor_port:
                    if distance_new != self.table[port].distance:
                        self.table[port].distance = distance_new
                        updated = True
                else:
                    if distance_new < self.table[port].distance:
                        self.table[port].next_hop = neighbor_port
                        self.table[port].distance = distance_new
                        updated = True
        return updated
    ...
    def lookup(self, port):
        if port in self.table:
            return self.table[port].next_hop, \
    ...
    def offline(self, port):
        for dst_port, item in self.table.items():
            if dst_port == port or item.next_hop == port:
    ...
    def __str__(self):
        ...
        for port, item in self.table.items():
            ...
            rows.append(f'  | {port:^6}  {next_hop:^7}  {dist:^4} |')
        rows.append(line)
        return '\n'.join(rows)
...
class DVR:
    ...
    def parse_commands(self, commands):
        ...
        if commands[0] == 'T':  # Display current routing table
            ...
            port = int(commands[1])
            next_hop, distance = self.route_table.lookup(port)
            if next_hop is None or distance == self.route_table.inf:
                ...
                print(f'[Data] Send data to next hop {next_hop}')
        ...
        while True:
            ...
            if data['type'] == 'table':  # Receive a routing table
                ...
                if data['dst'] == self.port:  # The router is the dst
                    ...
                    next_hop, distance = self.route_table.lookup(data['dst'])
                    if next_hop is None or distance == self.route_table.inf:
                        print(f'[Data] {port} is unreachable')
                    else:
                        data['path'].append(self.port)
                        self.socket.sendto(pickle.dumps(
                            data), (self.ip, next_hop))
    ...
    def reset_timer(self, port):
        ...
        if self.neighbors[port] is not None:
            ...
            print(f'Router {port} online')

        def timeout(port):
            print(f'Router {port} offline')
